...
class TestingNurseSupabaseUser extends BaseAuthUser {
  TestingNurseSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> testingNurseSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TestingNurseSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
